...
export function OrderList({ onViewDetail }: Props) {
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 16px' }}>
      ...
      {error && (
        <div style={{ textAlign: 'center', padding: 32 }}>
          ...
          <button
            onClick={load}
            style={{ padding: '8px 20px', cursor: 'pointer', border: '1px solid #d1d5db', borderRadius: 8, background: '#fff' }}
          >
            {t('orderList.retry')}
          </button>
        </div>
      ...
      {!loading && !error && orders.length === 0 && (
        <div style={{
          textAlign: 'center', padding: 64, color: '#9ca3af',
