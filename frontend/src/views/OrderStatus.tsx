...
export function OrderStatus({ order, onBack }: Props) {
  const { t, lang } = useTranslation();
  const locale = lang === 'pt' ? 'pt-BR' : 'en-US';

  const orderId   = `#ORD-${order.id.slice(0, 4).toUpperCase()}`;
  const date      = new Date(order.created_at).toLocaleString(locale, {
    day: '2-digit', month: '2-digit', year: 'numeric',
    hour: '2-digit', minute: '2-digit',
  });
  const qty       = order.quantity;
  const qtyLabel  = lang === 'pt'
    ? `${qty} ${qty === 1 ? 'unidade' : 'unidades'}`
    : `${qty} ${qty === 1 ? 'unit' : 'units'}`;
  const total     = ((order.product_price ?? 0) * qty)
    .toLocaleString(locale, { style: 'currency', currency: 'BRL' });

  const details = [
    [t('orderStatus.order'),    orderId],
    [t('orderStatus.product'),  order.product_name ?? '—'],
    [t('orderStatus.quantity'), qtyLabel],
    [t('orderStatus.date'),     date],
  ];
      ...
      <div className="flex items-start justify-center p-6 sm:p-10">
        <div className="w-full max-w-[560px] bg-white rounded-2xl shadow-lg px-8 sm:px-10 py-10 pb-8">
          ...
          <div className="flex justify-center mb-3">
            <div className="w-20 h-20 rounded-full bg-emerald-50 flex items-center justify-center text-3xl text-emerald-500">
              ✓
            </div>
          </div>
